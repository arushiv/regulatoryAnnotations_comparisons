...
# chrom   proxyStart      proxyEnd        pos        gene_id gene_name       tss_distance    ref     alt     snp     maf     pval_nominal    slope   qval    Position        Alleles cell    annotation
def getOpts():
...
def statevar_regress(d):
    d1 = d.drop_duplicates(['chrom','pos','gene_name','annotation'])
    d2 = d1.drop_duplicates(['chrom','pos','gene_name'])
...
def statevar_ld_regress(d):
    d1 = d.groupby(['chrom','pos','gene_name','annotation', 'absoluteBeta', 'absoluteDist', 'statevar']).size().reset_index(name='numLD')
    # print(d1)
    """
...
def fixForChromStates(d):
    ...
    print(len(d.index))
    # d = d.drop_duplicates(['chrom', 'proxyStart', 'proxyEnd', 'pos', 'gene_id'], keep=False)
    dtest = d[['chrom','pos','annotation']].drop_duplicates()
    subsetdf = dtest[dtest.duplicated(['chrom','pos'])]
    d = d[~((d['chrom'].isin(subsetdf['chrom'])) & (d['pos'].isin(subsetdf['pos'])))]
    print(len(d.index))
